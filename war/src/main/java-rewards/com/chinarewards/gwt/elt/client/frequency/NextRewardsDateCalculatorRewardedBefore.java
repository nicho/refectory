 ...
 * @since 0.2.0
 */
public class NextRewardsDateCalculatorRewardedBefore implements
	...
	public Date calNextRewardsDate(Date startTime, FrequencyClient frequency) {
		...
		if (frequency instanceof DayFrequencyClient) {
			...
			rewardsDate = DateTool.addSomeDay(startTime, dayInt);
			...
			if (min < weekDayInt) {
				rewardsDate = DateTool.addSomeDay(startTime, (weekInt - 1) * 7
						+ (7 + min - weekDayInt));
				...
				// 相等的情况
				rewardsDate = DateTool.addSomeDay(startTime, weekInt * 7);
			}
			...
			// ----加上几个月
			int rewardsMonth = startMonth + (monthInt - 1);
			...
			int rewardsYear = (yearInt - 1) + startYear;
			if (yearMonth > 12) {
				yearMonth = 12;
			}
			int rewardsMonth = yearMonth - 1;
