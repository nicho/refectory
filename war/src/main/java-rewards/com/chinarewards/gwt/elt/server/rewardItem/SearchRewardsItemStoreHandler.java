 ...
 */
public class SearchRewardsItemStoreHandler extends	BaseActionHandler<SearchRewardsItemStoreRequest, SearchRewardsItemStoreResponse> {
	...
	@Inject
	public SearchRewardsItemStoreHandler(RewardItemService rewardItemService) {
		this.rewardItemService = rewardItemService;
	}
	

	@Override
	public SearchRewardsItemStoreResponse execute(SearchRewardsItemStoreRequest request,
			ExecutionContext context) throws DispatchException {
		logger.debug(" request parameters: {}", request);

		SearchRewardsItemStoreResponse resp = new SearchRewardsItemStoreResponse();
		...
		PageStore<RewardItemStoreVo> items = rewardItemService.fetchRewardItemsStore(uc,model);
...
//		model.setRewardFrom(criteria.getRewardFrom());
	...
	private List<RewardsItemClient> adapter(List<RewardItemStoreVo> items,RewardItemService rewardsItemService) {
		List<RewardsItemClient> resultList = new ArrayList<RewardsItemClient>();

		for (RewardItemStoreVo item : items) {
			System.out.println("dddddddddddddddd="+item.getName());
			...
			client.setDegree(item.getItemStore().getDegree());
			...
			client.setCreateAt(item.getItemStore().getCreatedAt());
	...
	@Override
	public Class<SearchRewardsItemStoreRequest> getActionType() {
		return SearchRewardsItemStoreRequest.class;
	}

	@Override
	public void rollback(SearchRewardsItemStoreRequest arg0,
			SearchRewardsItemStoreResponse arg1, ExecutionContext arg2)
